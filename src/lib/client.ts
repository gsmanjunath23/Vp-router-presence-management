...
export default class Client extends EventEmitter {
  ...
  private handleConnectionPong = (payload: string) => {
    ...
    if (clientData.role === "mobile") {
      logger.info(`[PONG DETECTED] User ${this.id} responded to PING - refreshing presence`);
      presenceManager.refreshHeartbeat(this.id, (err) => {
        if (err) {
          logger.error(`Failed to refresh presence for ${this.id}:`, err);
        } else {
          debug(`Presence refreshed for ${this.id} via PONG`);
        }
      });
    }
  }
